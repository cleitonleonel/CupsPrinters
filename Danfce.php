...
class Danfce extends CommonNFePHP implements DocumentoNFePHP
{
    ...
    public function montaDANFCE($ecoNFCe = true)
    {
        ...
        $verProc  = $this->pSimpleGetValue($this->ide, 'verProc');
        ...
        */
        ...
        $this->html .= "<table width=\"90%\">\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td><img src=\"$this->logomarca\" width=\"75\" ></td>\n";
        $this->html .= "<td class=\"tLeft\" colspan=\"2\">".$emitCnpj."<br>".
                htmlspecialchars($emitRazao)."<br>".
                htmlspecialchars($emitLgr . ", nº" . $emitNro.", " .$emitCpl) . "<br>\n".
        ...
        $this->html .= "<table width=\"90%\">\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td class=\"tLeft\" width=\"55\">{$qtdItens} Itens</td>\n";
        //$this->html .= "<td class=\"tRight\">{$qtdItens}</td>\n";
        //$this->html .= "</tr>\n";
        //$this->html .= "<tr>\n";
        $this->html .= "<td class=\"tRight\"><strong>".htmlspecialchars('VALOR TOTAL R$')."</strong></td>\n";
        $this->html .= "<td class=\"tRight\"><strong>".number_format($vProd, 2, ',', '.')."</strong></td>\n";
        //$this->html .= "<strong>".htmlspecialchars('VALOR TOTAL R$ ').number_format($vProd, 2, ',', '.')."</strong></td>\n";
        $this->html .= "</tr>\n";
        $this->html .= "</table>\n";
        ...
        $this->html .= "</tr>\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td colspan=\"3\"><a class=\"consulta\" href=\"$urlQR\">$urlQR</a></td>\n";
        $this->html .= "</tr>\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td class=\"menor\" colspan=\"3\">{$chNFe}</td>\n";
        $this->html .= "</tr>\n";
        $this->html .= "</table>\n";
        ...
        $this->html .= "</tr>\n";        
        $this->html .= "</table>\n";
        $this->html .= "<table width=\"90%\" class=\"noBorder\">\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td colspan=\"3\" class=\"menor tCenter\">{$this->infCpl}</td>\n";
        $this->html .= "</tr>\n";
        $this->html .= "<tr>\n";
        $this->html .= "<td class=\"rodape tleft\">".str_replace(";", "<br>", $verProc) . "</td>\n";
        $this->html .= "</tr>\n";
        $this->html .= "</table>\n";
    ...
    protected function pagamento($pag){
        ...
        $item = 0;
        $detPagArray = $this->dom->getElementsByTagName("detPag");
        $newPag = $pag->item($item);
        //DADOS DE PAGAMENTO
        $pagHtml = "</tr>\n";

		foreach ($detPagArray as $pagI) {
		    if (isset($pagI)) {
                $tPag = $this->pSimpleGetValue($pagI,"tPag");
                $tPagNome = $this->tipoPag($tPag);
                $vPag = number_format($this->pSimpleGetValue($pagI,"vPag"), 2, ",", ".");
                $card = $pagI->getElementsByTagName("card")->item($item);
        ...
        $vTroco = number_format($this->pSimpleGetValue($newPag,"vTroco"), 2, ",", ".");
        if ( $vTroco != '0,00' ) {
           $pagHtml .= "<tr>\n";
    ...
    protected function tipoPag($tPag)
    {
        switch ($tPag) {
            ...
            case '15':
                ...
                break;
            case '16':
                $tPagNome = 'Deposito Bancário';
                break;
            case '17':
                $tPagNome = 'PIX';
                break;
            case '18':
                $tPagNome = 'Transf/Cart.Digital';
                break;
            case '19':
                $tPagNome = 'App Terceiros';
                break;
            case '99':
                $tPagNome = 'Outros';
        }
    ...
    protected function itens($det)
    {
        ...
        foreach ($det as $detI) {
            ...
            $infAdProd  = $thisItem->getElementsByTagName("infAdProd")->item(0)->nodeValue;
            ...
            if (!empty($infAdProd)) {
               ...
               $itensHtml .= "<td class=\"tLeft\" colspan=\"5\">".htmlspecialchars($infAdProd)."</td>\n";
	       $itensHtml .=  "</tr>\n";
     ...
     * @return string
     */
    private function imgQR($seq, $dimensao = 85)
    {
